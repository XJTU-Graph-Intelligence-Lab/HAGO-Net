...
class fea_emb(torch.nn.Module):
    def __init__(self, hidden_channels):
        super(fea_emb, self).__init__()
...
class area_emb(torch.nn.Module):
    def __init__(self, num_radial, cutoff=5.0, envelope_exponent=5):
        super(area_emb, self).__init__()
        self.cutoff = cutoff
        self.envelope = Envelope(envelope_exponent)
    ...
    def forward(self, area):
        area = area.unsqueeze(-1) / self.cutoff
        return self.envelope(area) * (self.freq * area).sin()

class volume_emb(torch.nn.Module):
    def __init__(self, num_radial, cutoff=5.0, envelope_exponent=5):
        super(volume_emb, self).__init__()
        self.cutoff = cutoff
        self.envelope = Envelope(envelope_exponent)
    ...
    def forward(self, volume):
        volume = volume.unsqueeze(-1) / self.cutoff
        return self.envelope(volume) * (self.freq * volume).sin()
...
class angle_emb_2(torch.nn.Module):
    def __init__(self, num_spherical, num_radial, cutoff=5.0,
                 envelope_exponent=5):
        super(angle_emb_2, self).__init__()
        assert num_radial <= 64
        self.num_spherical = num_spherical
        self.num_radial = num_radial
        self.cutoff = cutoff
...
class torsion_emb(torch.nn.Module):
    ...
    #def forward(self, dist, angle, phi, idx_kj):
        ...
        #cbf = torch.stack([f(phi) for f in self.sph_funcs], dim=1)

        #out = (rbf[idx_p1].view(-1, n, k) * cbf.view(-1, n, 1)).view(-1, n * k)
        return out

class torsion_emb_2(torch.nn.Module):
    def __init__(self, num_spherical, num_radial, cutoff=5.0,
                 envelope_exponent=5):
        super(torsion_emb_2, self).__init__()
        assert num_radial <= 64
        self.num_spherical = num_spherical #
        self.num_radial = num_radial
        self.cutoff = cutoff
    ...
    def forward(self, angle, phi):
    #def forward(self, dist, angle, phi, idx_kj):
        cbf = torch.stack([f(angle, phi) for f in self.sph_funcs], dim=1)

        n, k = self.num_spherical, self.num_radial
        out = cbf.view(-1, n, n, 1).view(-1, n * n)
        #cbf = torch.stack([f(phi) for f in self.sph_funcs], dim=1)

        #out = (rbf[idx_p1].view(-1, n, k) * cbf.view(-1, n, 1)).view(-1, n * k)
        return out

class torsion_emb_3(torch.nn.Module):
    def __init__(self, num_spherical, num_radial, cutoff=5.0,
                 envelope_exponent=5):
        super(torsion_emb_3, self).__init__()
        assert num_radial <= 64
        self.num_spherical = num_spherical #
        self.num_radial = num_radial
        self.cutoff = cutoff
    ...
    def forward(self, dist, angle, phi, idx_kj, p_idx_a):
    #def forward(self, dist, angle, phi, idx_kj):
        dist = dist / self.cutoff
        rbf = torch.stack([f(dist) for f in self.bessel_funcs], dim=1)
        cbf = torch.stack([f(angle[p_idx_a], phi) for f in self.sph_funcs], dim=1)

        n, k = self.num_spherical, self.num_radial
        out = (rbf[idx_kj][p_idx_a].view(-1, 1, n, k) * cbf.view(-1, n, n, 1)).view(-1, n * n * k)
        #cbf = torch.stack([f(phi) for f in self.sph_funcs], dim=1)
...
class torsion_emb(torch.nn.Module):
    ...
    def forward(self, dist, angle, phi, idx_p1):
        dist = dist / self.cutoff
        rbf = torch.stack([f(dist) for f in self.bessel_funcs], dim=1)
        # rbf = self.envelope(dist).unsqueeze(-1) * rbf
        cbf = torch.stack([f(phi) for f in self.sph_funcs], dim=1)
